<template>
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-6">
        <img
          src="https://image.freepik.com/free-vector/courier-with-parcel-background-delivery-service-van_165429-45.jpg"
          alt=""
        />
      </div>
      <div class="col-md-3"></div>
    ...
    <div class="row">
      ...
      <b-button @click="getToken()" pill variant="outline-danger"
        >search</b-button
      >
    </div>
    <div class="row mt-3">
      <div class="col-md-12" >
        <b-card  v-if="singnature!=null" :title="_track" tag="article">
          <b-list-group >
            <h4>Status :</h4>
            <b-list-group-item v-for="list in msg" :key="list.bar">
              <h4>{{ list.status_description }} : {{ list.location }}</h4>
              : <span>{{ list.status_date }} </span>
            </b-list-group-item>
          </b-list-group>
          <h4 v-if="singnature!=null">ผู้รับ : {{ receiver }}</h4>
          <br />
  ...
  data() {
    return {
      ...
      tokencate:
        "Token" +
        " " +
        "X2DkV+NtQvBbC4D=JlKjTHBuCKV8L3PYRcH-DZYzR=QIDkYnO=T=OKRGHZW0RGGICGATTyUJVxY+P^A4ROEcVVFyTuRCUyV^IrZz",
      barcode: [],
      token: "",
      track: "",
      receiver: null,
  ...
  methods: {
    ...
    getItem() {
      ...
      axios
        ...
        .catch((err) => {
          console.log(err);
          alert("ERROR404 Track ID ไม่ถูกต้อง");
